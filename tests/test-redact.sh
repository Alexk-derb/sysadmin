...
# D1. Второй секрет в той же строке. Правка C1 переносила остаток строки целиком
# и прекращала разбор — соседний секрет уходил открытым текстом. Класс A,
# внесённый лечением класса B (находка круга 2).
out=$(printf 'Environment=DB_PASSWORD=%s API_TOKEN=%s\n' "$CANARY" "$CANARY" | redact_stream)
leaked "D1 второй секрет в строке замаскирован" "$out"
out=$(printf 'ExecStart=/usr/bin/app --token=%s --port=8080 --password=%s --debug\n' "$CANARY" "$CANARY" | redact_stream)
leaked "D1 третий секрет в строке замаскирован" "$out"
kept   "D1 порт между двумя секретами сохранён" "$out" '--port=8080'
kept   "D1 хвост после второго секрета сохранён" "$out" '--debug'
# D2. Граница «следующее присваивание» применима ТОЛЬКО к env-виду (`=`).
# В YAML-скаляре `option=…` — часть значения, а не соседнее присваивание; общее
# правило снова открывало дефект A1 (находка круга 2).
out=$(printf 'password: %s option=%s\n' "$CANARY" "$CANARY" | redact_stream)
leaked "D2 YAML-скаляр не режется по знаку равенства" "$out"
out=$(printf 'api_key: %s ; comment=%s\n' "$CANARY" "$CANARY" | redact_stream)
leaked "D2 YAML-скаляр с комментарием не режется" "$out"
# Ключ НЕ в начале строки (элемент списка YAML). Без этого случая условие
# `sep == "="` не проверялось вовсе: ключ с начала строки не доходит до ветки
# командной строки в принципе, и мутант «применять и к двоеточию» набор
# пропускал — правило числилось покрытым напрасно.
out=$(printf -- '- api_key: %s option=%s\n' "$CANARY" "$CANARY" | redact_stream)
...
# D3. Закрытие блока принимается только строкой-маркером ЦЕЛИКОМ — как и открытие.
# Диагностическая строка с тем же литералом внутри объявляла блок закрытым и
# молча удалялась, а отказ становился нулевым (находка круга 2).
out=$(set +o pipefail; printf '%s\n' '-----BEGIN PRIVATE KEY-----' 'CANARYKEYBODY' \
      'nginx: expected -----END PRIVATE KEY----- before EOF' 'volume: pgdata 42G' | redact_stream 2>/dev/null; echo "rc=$?") # gitleaks:allow
kept "D3 диагностическая строка с литералом END сохранена" "$out" 'expected'
kept "D3 секция после неё сохранена"                       "$out" 'pgdata 42G'
case "$out" in *CANARYKEYBODY*) bad "D3 тело ключа утекло";; *) ok "D3 тело ключа не утекло";; esac
case "$out" in *"rc=0"*) bad "D3 блок объявлен закрытым по подстроке — отказ проглочен";; *) ok "D3 блок не закрыт — ненулевой код возврата";; esac
