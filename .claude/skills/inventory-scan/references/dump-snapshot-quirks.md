...
- **Имя сверяется по СЕГМЕНТАМ** (`_`, `-`, `.`, граница camelCase), а не
  подстрокой: `KEY` ловит `AWS_ACCESS_KEY_ID` и `authToken`, но больше не ловит
  `KEYBOARD_LAYOUT` и `MONKEY_MODE`. Хвостовой сегмент из белого списка
  (`*_URL`, `*_HOST`, `*_PATH`, `*_FILE`, `*_VERSION`, `*_TIMEOUT`, …) снимает
  подозрение с ИМЕНИ; секрет внутри такого значения по-прежнему ловится по
