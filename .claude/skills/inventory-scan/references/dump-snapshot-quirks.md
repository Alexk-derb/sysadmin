...
- **Незакрытый PEM-блок больше не съедает поток.** Блок начинается только
  строкой-маркером целиком (сообщение об ошибке nginx с тем же литералом внутри
  кавычек его не открывает), тело ограничено видом тела ключа, а при обрыве
  маска выходит из блока, предупреждает в stderr и **сохраняет остальные строки**.
  До v3 одна такая строка превращала шесть строк секции в одну — при коде
  возврата ноль и без признака отказа.
