...
- **Одно ядро правил, две точки входа.** Правила живут в единственном проходе
  ...
  место. Своих регулярных выражений у JSON-ветки нет: набор правил один. До v3
  это были две независимые реализации с двумя чёрными списками, и они разошлись
  по `signature`, `requirepass`, SQL `PASSWORD '…'` и двоеточию.

  **Точная формулировка гарантии.** Общее ядро гарантирует одинаковые ПРАВИЛА,
  но не одинаковый вход: JSON-ветка передаёт содержимое в кавычках (`@json`), и
  однажды это уже приводило к разному результату на одном и том же содержимом —
  ядро уходило в ветку «внутри строки» и обрубало командную строку, которую
  построчный путь сохранял. Лечение: контекст (начало строки или начало
  строки-значения) ядро вычисляет ОДИНАКОВО в обоих случаях. Совпадение веток
  проверяется тестами на общем корпусе — и по замаскированному, и по
  сохранённому, потому что первое совпадало и при расхождении второго.
- **Границу значения задаёт контекст, а не первый пробел.** Значение в кавычках
  маскируется внутри кавычек; голый литерал JSON и flow-отображение YAML
  (`{k: v, …}`) кончаются запятой или скобкой — в JSON замена берётся в кавычки,
  чтобы документ остался разбираемым. Незакавыченное значение при `=` идёт до
  конца строки, **если присваивание начинает свой контекст** (вид `printenv`;
  дефис элемента списка YAML считается частью начала), и до пробела, если не
  начинает (вид командной строки: `ExecStart=… --token=X --port=8080`,
  `Environment=A=1 B=2`). При `:` вне flow-контейнера — всегда до конца строки:
  в YAML-скаляре знак равенства часть значения. Тело многострочного YAML-блока
  (`key: |`) маскируется целиком — именно через него в 2026-08-08 утёк
  `sb_secret_`.
...
**`redact_json_with_jq` — устаревшее имя, псевдоним `redact_json_deep`.** Своей
реализации у неё больше нет: прежняя маскировала только массивы `.Env` и несла для
этого **собственный** чёрный список секрет-слов — то есть второй источник правил
рядом с утверждением «источник один». Ровно такой второй список и породил дефект
A6 (ветки разошлись незаметно), поэтому список убран, а не переписан.

Контракт (теперь общий с `redact_json_deep`): `3` — нет `jq`, `4` — `jq` не
