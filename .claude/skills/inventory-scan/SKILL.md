...
- **«секреты в containers-inspect.json»** — ИСПРАВЛЕНО (redaction v1). Скрипт
  ...
  остаются для аудита.
- **«маска пропускала секреты и стирала данные»** — ИСПРАВЛЕНО (redaction v3, 2026-08-22).
  Слепая сверка двумя моделями подтвердила замером восемь дефектов при зелёном наборе:
  значение обрывалось на первом пробеле, имя сверялось подстрокой (`KEYBOARD_LAYOUT`
  уезжал в инвентарь как `<REDACTED>`), одна строка с литералом `BEGIN PRIVATE KEY`
  съедала остаток потока при коде возврата ноль, а две ветки маски имели два разных
  чёрных списка. Лечение — одно ядро правил на обе ветки и границы из синтаксиса, а не
  из перечисления: ADR-0029 `decisions/0029-one-redaction-core-two-entry-points.md`.
  Задание с воспроизведением — `todos/2026-08-22-redact-defects-TASK.md`.
