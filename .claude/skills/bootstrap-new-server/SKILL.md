...
**Опциональное чтение `agent-config.json` (мозг).** На свежем сервере конфига обычно **нет** — это норма (агент ещё не настроен под этого оператора). Скилл — OPTIONAL-режим: если конфиг есть локально (например, оператор уже настраивал агент для другого сервера), скилл подхватит из него `language` и `operator.timezone`.
...
1. **Сначала — `/sysadmin-init`.** Создаст `agent-config.json` (мозг: менеджер паролей)
   + `infra-config.json` (карта: нужен ли мониторинг, куда складывать бэкапы, какой Telegram-бот).
...
- **«bootstrap → сразу setup-backups, без `/sysadmin-init`»** — оператор пропустил
  ...
  работать (нет конфига инфры `infra-config.json`). Урок: после bootstrap **первым** идёт
