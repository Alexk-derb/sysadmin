...
check_layer_ttl() {
    local layer="$1" want="$2" pair num
    # Срок привязывается К СЛОЮ, а не ищется рядом: «_live/ — 60 дн (было 14 дн)» проходил,
    # потому что искомое 14 в окне присутствовало, а оба числа канонические (сверка 2026-08-20).
    # Проверяются оба порядка написания: «слой — N дн» и «N дн для слоя».
    while IFS= read -r pair; do
        [ -z "$pair" ] && continue
        num="$(printf '%s' "$pair" | grep -oE '[0-9]+ *дн' | head -1 | grep -oE '[0-9]+')"
        [ -z "$num" ] && continue
        if [ "$num" != "$want" ]; then
            note_fail "C4: слою ${layer} приписан срок ${num} дн, канон ${want}; фрагмент: ${pair}"
            BAD_TTL=$((BAD_TTL+1))
        fi
    done < <(printf '%s' "$TTL_FLAT" | grep -oE "${layer}[^0-9]{0,25}[0-9]+ *дн")

    # Обратный порядок («N дн для слоя») намеренно НЕ проверяется привязкой: в перечислении
    # «_live/ 14 дн / _reference/ 60 дн» он приписывает слою срок соседа. Остаточная дыра
    # названа честно: инвертированная формулировка с КАНОНИЧЕСКИМ, но чужим числом при
    # наличии верного рядом — не ловится; внеканонное число там ловит проверка ниже.

    # Дополнительный слой обороны: любой срок рядом с именем слоя обязан быть каноническим.
    while IFS= read -r num; do
        [ -z "$num" ] && continue
        case "$num" in
            14|60|365) ;;
            *)
                note_fail "C4: рядом со слоем ${layer} стоит внеканонный срок ${num} дн"
                BAD_TTL=$((BAD_TTL+1))
                ;;
        esac
    done < <(printf '%s' "$TTL_FLAT" | grep -oE ".{0,40}${layer}.{0,40}" | grep -oE '[0-9]+ *дн' | grep -oE '[0-9]+')
    return 0
}
