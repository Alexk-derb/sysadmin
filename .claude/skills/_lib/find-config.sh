...
# Цель выездного указателя — строже, чем «похоже на корень»: ровно тот же состав, что
# требует писатель (`write-bridge.sh`). Расхождение писателя и читателя уже стоило круга
# сверки: читатель довольствовался одним CLAUDE.md, и чужой проект по устаревшему пути
# грузился как мозг sysadmin (сверка 2026-08-21, круг 8).
_is_bridge_target() {
    [ -f "$1/CLAUDE.md" ] || return 1
    [ -f "$1/.sysadmin-root" ] || [ -d "$1/.claude/skills" ]
}
...
locate_sysadmin_root() {
    ...
    if [ -f "$bridge" ]; then
        ...
        local from_bridge
        # Канонический способ: путь стоит отдельной выделенной строкой `**`<путь>`**` —
        # её пишет write-bridge.sh. Имя каталога при этом ЛЮБОЕ: репозиторий разрешено
        # клонировать не только в «sysadmin/» (сверка 2026-08-20 — писатель принимал любое
        # имя, а читатель требовал окончания на sysadmin, и указатель молча не читался).
        from_bridge=$(grep -oE '^\*\*`[^`]+`\*\*$' "$bridge" 2>/dev/null \
            | head -1 \
            | sed 's|^\*\*`||; s|`\*\*$||; s|[/\\]$||')
        # Запасной разбор для указателей старого формата: любой путь в обратных кавычках,
        # оканчивающийся на sysadmin.
        if [ -z "$from_bridge" ]; then
        ...
        # его пути теперь лежит что угодно. Требуем РОВНО ТО ЖЕ, что требует писатель:
        # CLAUDE.md плюс второй признак корня. Раньше здесь стоял только CLAUDE.md, хотя
        # комментарий обещал равенство с писателем, — и любой чужой проект с CLAUDE.md по
        # устаревшему пути загружался как мозг sysadmin (сверка 2026-08-21, круг 8).
        if [ -n "$from_bridge" ] && [ -d "$from_bridge" ] && _is_bridge_target "$from_bridge"; then
