...
EOF
    ...
    # Проверяется тот же состав, что и на временном файле. Иначе диверсант круга 7 проходит:
    # если подмена не состоялась, а СТАРЫЙ указатель ведёт на тот же путь, но с испорченным
    # frontmatter, проверка «непустой и путь внутри» принимает чужой файл за свежий и печатает
    # успех.
    if [ ! -s "$bridge" ] \
