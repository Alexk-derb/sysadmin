...
# применяется к временному файлу ДО подмены, к цели ПОСЛЕ неё и в самопроверке установки —
# иначе рубежи расходятся, и более мелкий пропускает то, что поймал бы более глубокий
# (круги 7 и 8).
_wb_check_bridge() {
    local file="$1" root="$2" fm desc
    [ -s "$file" ] || return 1
    fm="$(_wb_frontmatter "$file")" || return 1
    # Frontmatter канонического указателя — РОВНО три однострочных ключа. Проверка «ключи те
    # же» без проверки количества строк пропускала и лишние строки, и блочные скаляры
    # (сверка круга 9).
    [ "$(printf '%s\n' "$fm" | grep -c '')" = "3" ] || return 1
    ...
    # `description` обязателен по контракту субагентов и участвует в автоделегации. Проверки
    # `^description: .+` мало: `description: |` — непустая СТРОКА, но пустое ЗНАЧЕНИЕ (YAML
    # разбирает блочный скаляр без тела как ""), а `description: [unterminated` вовсе рвёт
    # разбор блока. Диверсант круга 9. Поэтому значение обязано быть однострочным простым
    # скаляром: не начинаться с индикатора YAML и содержать осмысленный текст.
    desc="$(printf '%s\n' "$fm" | grep -m1 '^description: ')" || return 1
...
# Годен ли КАТАЛОГ на роль цели указателя. Тот же состав, что у читателя
# (`_is_bridge_target` в find-config.sh): CLAUDE.md плюс второй признак корня. Держится
# отдельной функцией, чтобы писатель, читатель и самопроверка спрашивали одно и то же —
# расхождение этих трёх мест уже стоило двух кругов сверки (круги 8 и 9).
_wb_target_ok() {
    [ -d "$1" ] || return 1
...
write_bridge() {
    ...
    # Признак корня спрашивается у ОДНОЙ функции (`_wb_target_ok`), которая совпадает с
    # предикатом читателя: три места с собственными копиями правил уже разъезжались (круги
    # 8 и 9). Сообщения при этом остаются разными — оператору важно, чего именно не хватает.
    if ! _wb_target_ok "$sysadmin_root"; then
...
EOF
    ...
    # Проверяется и сам КАТАЛОГ: между входной проверкой и этой точкой он мог перестать быть
    # корнем мозга (снесли `.claude/`, переименовали репозиторий), и тогда указатель ведёт
    # туда, откуда читатель его уже не примет — окно круга 9.
    ...
    if ! _wb_check_bridge "$bridge" "$sysadmin_root"; then
        fail_reason="после записи $bridge не проходит проверку"
    elif ! _wb_target_ok "$sysadmin_root"; then
        fail_reason="за время записи $sysadmin_root перестал быть корнем мозга"
    fi
    if [ -n "$fail_reason" ]; then
        echo "write_bridge: $fail_reason — считаю неудачей." >&2
