...
#     пробную строку `имя=ПРОБА`.
#
# ЧТО ЭТО НЕ ГАРАНТИРУЕТ. Формулировка «веткам нечем разойтись» была здесь
# написана и дважды опровергнута замером: общее ядро даёт одинаковые ПРАВИЛА, но
# точки входа передают ему разное лексическое представление (JSON-ветка — в
# кавычках), а структурная секретность контейнера в глубокой ветке решается вне
# ядра. Оба случая исправлены и закрыты тестами, которые сверяют ветки на одном
# входе И по замаскированному, И по сохранённому. Но полноты разбора четырёх
# грамматик (env, командная строка, YAML, JSON-строка) это решение НЕ обещает —
# см. ADR-0029, раздел «Граница этого решения», и задание
# todos/2026-08-22-redact-grammar-modes-TASK.md.
#
...
# В YAML одинарная кавычка экранируется УДВОЕНИЕМ (`'a''b'`), а не обратной
# косой: первая кавычка пары принималась за конец значения, и хвост секрета
# оставался открытым (круг 4).
function find_close(s, i, q,   c) {
    while (i <= length(s)) {
        c = substr(s, i, 1)
        if (c == "\\" && q == "\"") { i += 2; continue }
        if (c == q) {
            if (q == "'"'"'" && substr(s, i + 1, 1) == "'"'"'") { i += 2; continue }
            return i
        }
        i++
    }
...
# содержимое объемлющей строки-значения. Дефис элемента списка YAML считается
# частью начала.
#
# Кавычка учитывается только как ОТКРЫВАЮЩАЯ, и это не формальность: прежняя
# редакция брала последнюю кавычку без разбора, и после закрытого аргумента
# (`env "MODE=check" API_TOKEN=…`) хвост из пробелов делал следующее
# присваивание «началом строки-значения» — остаток строки уничтожался (круг 4).
function ctx_atstart(t,   i, c, st, start, tail) {
    i = 1; st = 0; start = 0
    while (i <= length(t)) {
        c = substr(t, i, 1)
        if (c == "\\") { i += 2; continue }
        if (c == "\"") {
            if (st) { st = 0; start = 0 } else { st = 1; start = i }
        }
        i++
    }
    tail = (st && start > 0) ? substr(t, start + 1) : t
    if (!st) tail = t
    return (tail ~ /^[ \t]*(-[ \t]+)?$/)
}
...
#
# Одинарные кавычки учитываются наравне с двойными: `{note: '}', password: X}`
# иначе считался закрытым на скобке ВНУТРИ строки, и всё после секрета
# уничтожалось (круг 4).
function flow_depth(t,   i, c, dq, sq, d) {
    i = 1; dq = 0; sq = 0; d = 0
    while (i <= length(t)) {
        c = substr(t, i, 1)
        if (c == "\\" && dq) { i += 2; continue }
        if (c == "\"" && !sq) dq = 1 - dq
        else if (c == "'"'"'" && !dq) sq = 1 - sq
        else if (!dq && !sq) {
            if (c == "{" || c == "[") d++
            else if (c == "}" || c == "]") d--
        }
...
# Индекс закрывающей скобки, парной открывающей в позиции i (строки учитываются).
# 0 — не найдена. Нужен, чтобы контейнер под секретным именем маскировался
# ЦЕЛИКОМ: у элементов массива нет собственных имён, и «разберётся по своим
# именам» для них не работает — секрет уходил открытым текстом (круг 4).
function find_close_bracket(s, i,   c, o, cl, d, dq, sq) {
    o = substr(s, i, 1)
    cl = (o == "{") ? "}" : "]"
    d = 0; dq = 0; sq = 0
    while (i <= length(s)) {
...
function scan_names(s,   out, rest, m, pre, nm, sep, nq, instr, c, j, val, tail, guard, atstart, depth) {
    ...
    while (match(rest, ASSIGN)) {
        ...
        c = substr(rest, 1, 1)
        if (c == "") break
        # Значение-контейнер под НЕсекретным именем не трогаем: строки внутри
        # разбираются своими именами и правилами по значению. Под СЕКРЕТНЫМ —
        # маскируется целиком: у элементов массива собственных имён нет, и
        # `{"DB_PASSWORD":["x","y"]}` уходил открытым текстом, тогда как глубокая
        # ветка его маскировала — ветки расходились (круг 4). Замена строкой
        # оставляет документ разбираемым.
        if (c == "{" || c == "[") {
            j = find_close_bracket(rest, 1)
        ...
        #
        # ИСКЛЮЧЕНИЕ: разделитель `=`, и присваивание стоит НЕ в начале строки.
        # Это вид командной строки (`ExecStart=… --token=X --port=8080`,
        # `Environment=A=1 B=2` из `systemctl cat`, `export NAME=v arg`), где
        # значение кончается пробелом, а «до конца строки» уничтожает соседей —
        # тихо и с кодом возврата ноль. Присваивание С НАЧАЛА строки — вид
        # `printenv`, там значение идёт до конца строки, и обрыв на пробеле
        # оставлял бы хвост пароля в снимке (дефект A1).
        #
        # Разбор при этом ПРОДОЛЖАЕТСЯ, а не прекращается: иначе второй секрет в
        # той же строке уходит открытым текстом. И только для `=`: в YAML-скаляре
        # (`password: значение option=x`) знак равенства — часть ЗНАЧЕНИЯ.
        # Обе ошибки были внесены первой редакцией этого исключения (круг 2).
        if (sep == "=" && !atstart) {
            match(rest, /^[^ \t]*/)
            out = out R
...
BEGIN {
    ...
    PEMONLY = "^[ \t]*" PEMB "[ \t]*$"
    # Закрытие принимается ТОЛЬКО строкой-маркером целиком — как и открытие.
    # Проверка подстрокой объявляла закрытием диагностическую строку с тем же
    # литералом («nginx: expected -----END PRIVATE KEY----- before EOF») и молча
    # её удаляла, а отказ при этом становился нулевым (находка круга 2).
    PEMEONLY = "^[ 	]*" PEME "[ 	]*$"
...
END {
    if (inkey) {
        broken = 1
        if (!warned) print "redact: поток оборвался внутри блока PRIVATE KEY — тело замаскировано" > "/dev/stderr"
    }
    # Блок открылся и не закрылся маркером END. Проглоченные строки неотличимы
    # от полезного однострочного вывода (`docker ps -q`), доказать их природу
    # нельзя — значит отказ обязан быть ГРОМКИМ. Задание разрешает ровно две
    # реакции: сохранить остальные строки либо явно отказать ненулевым кодом;
    # молчаливая потеря при коде возврата ноль запрещена.
    if (broken) exit 3
}
'
}
...
#
# Код возврата 3 — незакрытый блок PRIVATE KEY: часть строк проглочена, доказать
# их природу нельзя. Вызывающий обязан считать секцию НЕПОЛНОЙ. Молчаливо
# продолжать нельзя, поэтому отказ и сделан кодом, а не только предупреждением.
redact_stream() { _redact_core 0; }

# То же, но отказ маскировки (rc=3) не роняет вызывающего под `set -e`.
# Нужен там, где обрыв на полпути хуже неполного вывода: ротация секрета,
# оборванная между сменой пароля и перезапуском потребителя, оставляет систему
# в половинчатом состоянии. Настоящие ошибки (упавший ssh, недоступный демон)
# по-прежнему пробрасываются — понижается ТОЛЬКО код 3.
redact_stream_soft() {
    local rc=0
    _redact_core 0 || rc=$?
    if [ "$rc" = 3 ]; then
        echo "redact: вывод неполон — незакрытый блок PRIVATE KEY, часть строк не перенесена" >&2
        return 0
    fi
    return "$rc"
}
...
redact_json_deep() {
    ...
    # Два параллельных списка одного и того же набора имён (порядок задаёт
    # `unique`, он одинаков для обоих вызовов): оригиналы в JSON-виде и они же,
    # НОРМАЛИЗОВАННЫЕ под пробу. Нормализация — не косметика: имена вида
    # `db password` и `password[]` реально встречаются, а отбрасывание их по
    # набору символов означало «имя не секретное» и выпускало значение открытым
    # текстом (находка контрольного круга). Заодно нормализация убирает переводы
    # строк, которые сломали бы соответствие 1:1.
    if ! printf '%s' "$doc" | jq -r '
            [paths[] | select(type == "string")] | unique | .[] | @json
    ...
    if ! printf '%s' "$doc" | jq -r '
            ...
            | gsub("[^A-Za-z0-9_.-]"; "_")
        ' > "$tmpd/keysan" 2>/dev/null; then
        rm -rf "$tmpd"; return 4
    fi
    : > "$tmpd/secretkeys"
    if [ -s "$tmpd/keys" ]; then
        sed 's/$/=REDACTPROBEZZ/' "$tmpd/keysan" | _redact_core 1 > "$tmpd/probe" 2>/dev/null
        nk=$(wc -l < "$tmpd/keys" | tr -d ' ')
        if [ "$nk" != "$(wc -l < "$tmpd/keysan" | tr -d ' ')" ] || \
           [ "$nk" != "$(wc -l < "$tmpd/probe" | tr -d ' ')" ]; then
            rm -rf "$tmpd"; return 4
        fi
        paste -d'\t' "$tmpd/keys" "$tmpd/probe" \
          | awk -F'\t' 'index($2, "REDACTPROBEZZ") == 0 { print $1 }' \
          > "$tmpd/secretkeys" 2>/dev/null
    fi
    ...
    printf '%s' "$doc" | jq --slurpfile O "$tmpd/orig" --slurpfile R "$tmpd/red" \
        ...
        # Имя для замаскированного ключа выбирается так, чтобы выполнялись ОБА
        # условия сразу, иначе лечение одного даёт второй дефект:
        #   * не зависело от порядка ключей в документе — иначе diff двух снимков
        #     одного объекта показывает различие, которого не было;
        #   * не сталкивалось ни с одним другим итоговым именем — документ,
        #     который УЖЕ содержит ключи `<REDACTED>` и `<REDACTED>~1` (повторная
        #     маскировка снимка — обычное дело), иначе терял запись целиком при
        #     коде возврата ноль.
        # Отсюда: номер берётся из порядка СОРТИРОВКИ исходных имён, а кандидаты,
        # занятые чужими именами, пропускаются.
        # $force — секретность РОДИТЕЛЯ. Контейнер под секретным именем не
        # подменяется целиком (структура снимка нужна оператору), но все его
        # скалярные листья маскируются: нейтральное внутреннее имя `value` не
        # отменяет секретности родителя `DB_PASSWORD` (находка контрольного круга).
        # Переименование и обход идут ОДНИМ проходом: решение «секретен ли ключ»
        # принимается по ИСХОДНОМУ имени, после переименования его уже не узнать.
        def rmap($m; $sk; $force):
          if type == "object" then
            . as $obj
            | (reduce (keys_unsorted[]) as $k ({}; .[($m[$k] // $k)] += [$k])) as $grp
            | (reduce (keys_unsorted[]) as $k ({}; .[($m[$k] // $k)] = true)) as $taken
            | reduce (keys_unsorted[]) as $k ({};
                (($m[$k] // $k)) as $nk
                | ($grp[$nk] | sort) as $orig
                | (if ($orig | length) == 1 then $nk
                   else
                     ([range(1; ($orig | length) + ($taken | length) + 2)]
                      | map("\($nk)~\(.)")
                      | map(select(. as $c | $taken | has($c) | not)))[$orig | index($k)]
                   end) as $out_k
                | .[$out_k] = ($obj[$k] | rmap($m; $sk; $force or ($sk[$k] // false)))
              )
          elif type == "array"  then map(rmap($m; $sk; $force))
          elif $force           then "<REDACTED>"
          elif type == "string" then ($m[.] // .)
          else . end;
        (reduce range(0; ($O | length)) as $i ({}; .[$O[$i]] = $R[$i])) as $map
        | (reduce ($S[]) as $k ({}; .[$k] = true)) as $skeys
        | rmap($map; $skeys; false)
    ' 2>/dev/null
    rc=$?
    rm -rf "$tmpd"
    return $rc
...
# УСТАРЕЛО: сохранено только как имя для внешних потребителей библиотеки —
# псевдоним redact_json_deep, живых вызовов в репозитории нет.
#
# Своей реализации у неё больше НЕТ намеренно. Прежняя маскировала только массивы
# `.Env` и несла для этого СОБСТВЕННЫЙ чёрный список секрет-слов — то есть второй
# источник правил рядом с утверждением «источник один». Ровно такой второй список
# и породил дефект A6 (ветки разошлись незаметно), поэтому он убран, а не
# переписан. Контрольный круг сверки назвал его прямо.
#
# Контракт (теперь общий с redact_json_deep): 3 — нет jq, 4 — jq не разобрал вход
# или вход пуст, 0 — успех. Раньше контракт держался ИСКЛЮЧИТЕЛЬНО на
# `set -o pipefail` вызывающего: код возврата брался у хвостового sed, и битый
# JSON давал ноль при пустом выводе (дефект B4).
redact_json_with_jq() { redact_json_deep; }
