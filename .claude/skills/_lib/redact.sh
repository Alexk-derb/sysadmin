...
function scan_names(s,   out, rest, m, pre, nm, sep, nq, instr, c, j, val, tail, guard) {
    ...
    while (match(rest, ASSIGN)) {
        ...
        #
        # ИСКЛЮЧЕНИЕ: в строке есть ещё одно присваивание env-вида. Так выглядят
        # `Environment=A=1 B=2` в unit-файлах (`systemctl cat`) и `ExecStart=` с
        # флагами — там «до конца строки» уничтожает соседние переменные, тихо и
        # с кодом возврата ноль (находка контрольного круга). Значение обрывается
        # перед следующим присваиванием.
        if (match(rest, NEXTASSIGN)) {
            out = out R substr(rest, RSTART)
            rest = ""
            break
        }
        out = out R; rest = ""; break
...
BEGIN {
    ...
    ASSIGN = "\"?[A-Za-z0-9_.-]+\"?[ \t]*[=:]"
    # Следующее присваивание env-вида в той же строке: пробел, затем имя (или
    # флаг `--имя`) и `=`. Именно оно ограничивает значение в строках вида
    # `Environment=A=1 B=2` и `ExecStart=… --token=X --port=8080`.
    NEXTASSIGN = "[ \t]+-{0,2}[A-Za-z_][A-Za-z0-9_.-]*="
