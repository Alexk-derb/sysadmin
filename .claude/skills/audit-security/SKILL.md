...
> Своя копия регулярки здесь жила до 2026-08-20 и требовала, чтобы путь начинался с `/` и
> оканчивался на `sysadmin`. Она ломалась и на родном виндовом пути, и на репозитории,
> склонированном в каталог с другим именем, — а расходилась с настоящим читателем молча.
