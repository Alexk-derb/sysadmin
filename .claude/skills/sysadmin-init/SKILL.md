...
if ! write_bridge "$SYSADMIN_ROOT"; then
    ...
    # Причина уже напечатана громко. NEVER рапортовать «Готово»: @sysadmin из чужих папок
    # работать не будет, а установка выглядела бы удавшейся (C.9).
    echo "STOP: bridge не создан — разберись с причиной выше и повтори шаг." >&2
    exit 1
fi
```

**Код возврата обязателен к проверке.** До 2026-08-20 helper вызывался без проверки, и его
отказ уходил в никуда: скилл продолжал работу и печатал успех. Теперь rc=1 означает, что
указателя нет.
