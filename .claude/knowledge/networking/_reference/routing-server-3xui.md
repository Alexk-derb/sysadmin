...
sources_checked:
  ...
  - эмпирика 2026-07-02 (Xray 26.6.27): AND-семантика domain+ip в одном правиле
---
...
> **🔴 НИКОГДА не сливай `domain` и `ip` в ОДНО правило — они матчатся по AND, не OR.**
> В Xray разнотипные условия внутри одного field-rule (`domain` + `ip` + `protocol` + …)
> объединяются по **логическому И**: правило срабатывает, только если совпали ВСЕ
> заданные типы. Поэтому РФ-direct обязан быть **двумя** правилами (эталон выше: правило
> 5 = только `ip:geoip:ru`, правило 6 = только `domain:[category-ru,...]`) — между
> разными правилами семантика OR, срабатывает любое.
>
> Если слить их в одно `{"domain":[...], "ip":["geoip:ru"], "outboundTag":"direct"}` —
> правило **тихо ломается**: доменное соединение из прокси приходит как `tcp:host:443`,
> клауза `domain` совпадает, но `ip:geoip:ru` НЕ выполняется (IP назначения на фазе
> доменного матчинга ещё не сопоставлен с geoip) → `AND(да,нет)=нет` → правило
> отваливается → трафик проваливается в default (catch-all) на upstream/США. Симптом:
> РФ-домены (yandex.ru, vk.com) уходят через зарубежный выход вместо direct, при этом
> ads-блок и другие чисто-domain-правила работают (сбивает с толку — «geosite читается,
> а РФ-правило нет»).
>
> **Эмпирика (Xray 26.6.27, 2026-07-02):** правило `domain+ip вместе` → `yandex.ru`
> уходит мимо direct; то же правило БЕЗ клаузы `ip` → `yandex.ru → direct`. Разнесение
> на два правила чинит. **При аудите чужого/дрейфнувшего конфига** ищи правила с
> непустыми `domain` И `ip` одновременно — это кандидат в тихий баг. `/configure-vpn-routing`
> перезаписывает `.routing` целиком по этому эталону, поэтому применение эталона закрывает
> класс; но ручные правки в панели/БД могут его вернуть.
